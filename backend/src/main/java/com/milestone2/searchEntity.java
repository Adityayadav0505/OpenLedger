...
/**
 * Servlet implementation class searchEntity
 * Mapped to /searchData.do in web.xml.
 */
public class searchEntity extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			// WHERE must come before LIMIT, and the LIKE pattern is bound as a parameter.
			String query = "SELECT * FROM invoice_details WHERE doc_id LIKE ? LIMIT ?,?;";
			PreparedStatement statement = connection.prepareStatement(query);
			statement.setString(1, invoice_id + "%");
			statement.setInt(2, (limit)*(count-1));
